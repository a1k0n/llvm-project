...
#define DEBUG_TYPE "mcs51-parser"
...
public:
  ...
  void addExpr(MCInst &Inst, const MCExpr *Expr) const {
    ...
    LLVM_DEBUG(dbgs() << "addExpr: "; Expr->dump());
...
OperandMatchResultTy MCS51AsmParser::parseImmediate(OperandVector &Operands) {
  LLVM_DEBUG(dbgs() << "parseImmediate: kind " << getLexer().getKind() << "\n");
  switch (getLexer().getKind()) {
  default:
    return MatchOperand_NoMatch;
  ...
  LLVM_DEBUG(dbgs() << "parseImmediate: MCExpr: "; IdVal->dump());
